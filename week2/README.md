...
## A2 warmup: Developing a C library for reading and writing numbers
...
As we saw in A1, most C libraries are structured as one or more header files and
one or more *implementation files* (`.c` files). The implementation file(s)
of a library *do not contain a `main()` function*.  
This enables us to compile a library *once* and use it
*multiple times* (more on this next week).
For now, as in A1, we will simply pass multiple `.c` files to
the compiler.
...
This week, you get no handout so that you get experience building
a working environment from scratch.
...
* Create a file `test_numlib.c`.

  This file must contain `#include "numlib.h"` and `"include "assert.h"`,
  a function `test_get_number()` that asserts whether get_number returns
  the expected number and a `main()`
  function. The `main()` function should call `test_get_number()` and print
  a message stating that the tests passed.

* Create a `Makefile` with a target `test_numlib` that compiles
  `numlib.c` *and* `numlib_test.c` together (by passing both to the C compiler)
  to produce a program called `test_numlib`. Look at A1 for a reference.

Now run `make numlib_test` and run `./test_numlib` to ensure
...
* Have a look at the Makefile comments in the solution for an introduction
to Makefile syntax. [Solution here](a2-solution/part1).
...
* Copy `get_digit` into `numlib.c` (nowhere else). 
  Also add includes for the the `stdio.h` and `ctype.h` headers in `numlib.c`.

* To get started, modify `get_number` such that it contains a loop that repeatedly
  ...
  This is not perfect, just a first step.

* Modify `test_numlib.c` such that it calls `test_get_number()` no longer
asserts anything, but print the number that `get_number` returns. You should call
test_get_number() twice in the `main` function (so that your test reads two
numbers from stdin).

Now ponder:

* Can `test_numlib.c` directly use `get_digit()`? Why or why not? What
  would you modify if you wanted this to be possible?

* What happens when you run `echo "123 456" | ./test_numlib`?
...
* Define the main function as
  ...
  See the exercises from week1 as a reference for how to manage command-line parameters.

* Use `fopen(filename, "r")` to open a file for reading. This function is introduced in
...
* Use the `fread()` function to actually perform the read from the
  ...
  the entire file. This function is introduced in the lecture on Thursday.
...
have written) - we start from scratch again.
...
Note that `le` stands for Little Endian, which is the representation of
...
* The programs `uint_ascii_to_uint_le` and `uint_le_to_uint_ascii` are inverses of each other.
A command such as
